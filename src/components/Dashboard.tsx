...
export default function Dashboard({ currentUser }: DashboardProps) {
  ...
  const totalMonthIncome = currentMonthTransactions.filter((t) => t.type === "income").reduce((sum, t) => sum + t.amount, 0);

  const totalMonthExpense = Math.abs(currentMonthTransactions.filter((t) => t.type === "expense").reduce((sum, t) => sum + t.amount, 0));
  ...
  const dynamicCategories = React.useMemo(() => {
    const existing = transactions.filter((t) => t.type === modalType).map((t) => t.category);
    const defaults = modalType === "expense" ? ["Infrastructure", "Meals", "Travel", "Software", "Marketing", "Operations"] : ["Consulting", "Salary", "Investments", "Revenue"];
    return Array.from(new Set([...defaults, ...existing]));
  }, [transactions, modalType]);

  const hasMonthTxs = currentMonthTransactions.length > 0;

  const expenseArcVal = hasMonthTxs && totalMonthExpense > 0 ? Math.max(10, Math.min(240, (totalMonthExpense / Math.max(1, totalMonthIncome)) * 251.2)) : 0;

  const incomeArcVal = hasMonthTxs && totalMonthIncome > 0 && surplus > 0 ? Math.max(50, Math.min(200, (surplus / Math.max(1, totalMonthIncome)) * 251.2)) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-[#e5e2e1]" id="dash-greeting-h2">
            Welcome, {currentUser.fullName}
          </h2>
          <p className="text-[#bbcabf] text-sm mt-1">
            Reviewing your financial precision for {currentMonth} {currentYear}.
          </p>
        </div>
          ...
          <div className="flex-1 flex items-end gap-4 h-48 mt-auto relative z-10 pt-6 border-b border-[#353534] pb-2">
            ...
            <div className="flex-1 flex flex-col justify-end h-full group/bar cursor-pointer ml-8 relative">
              <div className="w-full bg-[#2a2a2a] rounded-t relative h-full flex items-end hover:bg-zinc-800 transition-all">
                <div className="w-full bg-[#4edea3] rounded-t transition-all duration-700 block" style={{ height: `${Math.min(100, (w1Expense / maxWeekly) * 100)}%` }}></div>
            ...
            <div className="flex-1 flex flex-col justify-end h-full group/bar cursor-pointer relative">
              <div className="w-full bg-[#2a2a2a] rounded-t relative h-full flex items-end hover:bg-zinc-800 transition-all">
                <div className="w-full bg-[#4edea3] rounded-t transition-all duration-700 block" style={{ height: `${Math.min(100, (w2Expense / maxWeekly) * 100)}%` }}></div>
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 border border-white/10 text-white text-[10px] rounded opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-xl font-mono">
                  ${w2Expense.toFixed(2)}
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col justify-end h-full group/bar cursor-pointer relative">
              <div className="w-full bg-[#2a2a2a] rounded-t relative h-full flex items-end hover:bg-zinc-800 transition-all">
                <div className="w-full bg-[#4edea3] rounded-t transition-all duration-700 block" style={{ height: `${Math.min(100, (w3Expense / maxWeekly) * 100)}%` }}></div>
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 border border-white/10 text-white text-[10px] rounded opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-xl font-mono">
                  ${w3Expense.toFixed(2)}
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col justify-end h-full group/bar cursor-pointer relative">
              <div className="w-full bg-[#2a2a2a] rounded-t relative h-full flex items-end hover:bg-zinc-800 transition-all">
                <div className="w-full bg-[#4edea3] rounded-t transition-all duration-700 block" style={{ height: `${Math.min(100, (w4Expense / maxWeekly) * 100)}%` }}></div>
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 border border-white/10 text-white text-[10px] rounded opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20 shadow-xl font-mono">
                  ${w4Expense.toFixed(2)}
                </div>
              </div>
      ...
      <div className="bg-[#201f1f] rounded-xl border border-white/5 overflow-hidden flex flex-col">
          ...
          <div className="flex bg-[#353534] rounded p-1 border border-white/5">
            <button
              ...
              className={`px-4 py-1 rounded font-mono text-[11px] uppercase tracking-wider transition-all cursor-pointer ${
                filterType === "expense" ? "bg-[#1c1b1b] border border-white/10 text-white shadow-sm font-bold" : "text-[#bbcabf] hover:text-white"
              }`}
            >
              Expenses
            </button>
            <button
              ...
              className={`px-4 py-1 rounded font-mono text-[11px] uppercase tracking-wider transition-all cursor-pointer ${
                filterType === "income" ? "bg-[#1c1b1b] border border-white/10 text-white shadow-sm font-bold" : "text-[#bbcabf] hover:text-white"
              }`}
            >
              Income
            </button>
        ...
        <div className="overflow-x-auto">
          {recentFiltered.length === 0 ? (
            <div className="text-center py-12 text-[#bbcabf]/50 text-xs font-mono">No transactions added under {filterType} this month yet.</div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="text-[#e5e2e1] text-xs">
                {recentFiltered.map((t) => (
                  <tr key={t.id} className="h-12 border-b border-white/5 hover:bg-white/[0.02] transition-colors group cursor-pointer">
                    ...
                    <td className="py-2 px-6">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-mono lowercase ${getCategoryStyle(t.category)}`}>{t.category}</span>
                    </td>
                    <td className="py-2 px-6 text-right text-[#bbcabf] font-mono">{new Date(t.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="relative w-full max-w-lg bg-[#201f1f] border border-white/10 rounded-xl shadow-2xl overflow-hidden transform transition-all">
            ...
            <form onSubmit={handleSaveTransaction} className="p-6 space-y-5" id="frmAddTransaction">
              <div className="space-y-1.5">
                <label className="block text-xs uppercase tracking-wider font-mono text-[#bbcabf]">Title</label>
                <input
                  type="text"
                  value={title}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="block text-xs uppercase tracking-wider font-mono text-[#bbcabf]">Amount ($)</label>
                  <input
                    type="number"
                    value={amount}
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs uppercase tracking-wider font-mono text-[#bbcabf]">Date</label>
                  <input
                    type="date"
                    value={date}
              ...
              <div className="space-y-1.5">
                <label className="block text-xs uppercase tracking-wider font-mono text-[#bbcabf]">Category</label>
                <input
                  type="text"
