...
export default function AdminPage({ onUpdateUser }: AdminPageProps) {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((u) => u.username.toLowerCase().includes(q) || u.fullName.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || u.phone.includes(q));
  }, [users, searchQuery]);
  ...
  return (
    <div className="space-y-6 flex-1 font-sans relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-[#e5e2e1]" id="admin-view-title-h2">
            User Administration
          </h2>
            ...
            <tbody className="text-xs text-[#e5e2e1] [&>tr:nth-child(even)]:bg-white/[0.005]">
              {paginatedUsers.length === 0 ? (
                ...
                paginatedUsers.map((user) => {
                  ...
                  return (
                    <tr key={user.id} className={`border-b border-white/5 hover:bg-white/[0.015] transition-all group ${isEditing ? "bg-[#4edea3]/[0.02] border-[#4edea3]/20" : ""}`}>
                      {/* USERNAME CELL */}
                        ...
                        {isEditing ? (
                          <div className="relative">
                            ...
                            {validationErrors.username && (
                              <div className="absolute left-0 top-full mt-1 bg-[#93000a] text-[#ffdad6] text-[10px] py-1 px-2.5 rounded shadow-lg z-30 font-mono">{validationErrors.username}</div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="py-3 px-6">
                        {isEditing ? (
                          ...
                          <div className={user.isUnverified ? "text-[#bbcabf] italic" : ""}>{user.fullName}</div>
                        )}
                      </td>
                        ...
                        {isEditing ? (
                          <div className="relative">
                            ...
                            {validationErrors.email && (
                              <div className="absolute left-0 top-full mt-1 bg-[#93000a] text-[#ffdad6] text-[10px] py-1 px-2.5 rounded shadow-lg z-30 font-mono">{validationErrors.email}</div>
                            )}
                          </div>
                        ) : (
                        ...
                        {isEditing ? (
                          <div className="relative">
                            ...
                            {validationErrors.phone && (
                              <div className="absolute left-0 top-full mt-1 bg-[#93000a] text-[#ffdad6] text-[10px] py-1 px-2.5 rounded shadow-lg z-30 font-mono">{validationErrors.phone}</div>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="py-3 px-6 text-right relative">
                        {isEditing ? (
                          <div className="flex items-center gap-1 justify-end">
                            <button onClick={() => handleSaveEdit(user)} className="p-1 text-[#4edea3] hover:bg-[#4edea3]/10 rounded cursor-pointer transition-colors" title="Save details">
                              <Check className="w-4.5 h-4.5" />
                            </button>
                            <button onClick={handleCancelEdit} className="p-1 text-[#ffb4ab] hover:bg-rose-500/10 rounded cursor-pointer transition-colors" title="Discard edits">
                              <X className="w-4.5 h-4.5" />
                            </button>
                          </div>
                        ) : (
                          <div className="inline-block relative">
                            <button onClick={(e) => handleOpenMenu(user.id, e)} className="text-[#bbcabf] hover:text-[#4edea3] transition-all p-1 hover:bg-white/5 rounded-full cursor-pointer">
                              <MoreVertical className="w-4.5 h-4.5" />
                            </button>
                            {activeMenuUserId === user.id && (
                              <div ref={menuRef} className="absolute right-0 top-full mt-1.5 z-50 w-44 bg-[#1e1e1e] border border-white/10 rounded-lg shadow-2xl py-1 text-xs text-left">
                                <button
