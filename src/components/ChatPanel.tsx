...
const quickActions = ["📊 Analyze my spending", "💰 How much did I save?", "📈 Predict my cash flow", "🔥 Roast my spending!", "💡 Give me saving tips"];
...
export default function ChatPanel({ isOpen, onClose }: ChatPanelProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      {isOpen && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[60] md:bg-transparent md:backdrop-blur-none" onClick={onClose} />}
      ...
      >
                ...
                <div className="absolute right-0 top-full mt-1.5 bg-[#1e1e1e] border border-white/10 rounded-lg shadow-2xl py-1 z-50 w-48">
                  {personas.map((p) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 text-xs hover:bg-white/5 transition-all cursor-pointer ${persona === p.id ? "text-[#4edea3]" : "text-[#e5e2e1]"}`}
                    >
                      {p.label}
                    </button>
                  ))}
          ...
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#bbcabf] leading-relaxed">Ask me about your spending, add transactions by voice, or upload a receipt photo!</p>
              </div>
              ...
              <div className={`max-w-[80%] space-y-2 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div
                  className={`px-3.5 py-2.5 rounded-2xl text-xs leading-relaxed whitespace-pre-wrap ${
                    msg.role === "user" ? "bg-[#4edea3] text-[#003824] rounded-br-md font-medium" : "bg-white/[0.06] text-[#e5e2e1] rounded-bl-md border border-white/5"
                  }`}
                >
                  {msg.content}
                </div>
                        ...
                        {msg.parsedTx.map((tx, idx) => (
                          <div key={idx} className="bg-[#1e1e1e] border border-[#4edea3]/20 rounded-xl p-3 space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-1.5 text-[11px]">
                              ...
                              <div className={`font-bold ${tx.amount < 0 ? "text-[#ffb4ab]" : "text-[#4edea3]"}`}>${Math.abs(tx.amount).toFixed(2)}</div>
                      ...
                      <>
                        <div className="bg-[#1e1e1e] border border-[#4edea3]/20 rounded-xl p-3 space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-1.5 text-[11px]">
                            ...
                            <div className="text-[#bbcabf]">Amount</div>
                            <div className={`font-bold ${msg.parsedTx.amount < 0 ? "text-[#ffb4ab]" : "text-[#4edea3]"}`}>${Math.abs(msg.parsedTx.amount).toFixed(2)}</div>
                            <div className="text-[#bbcabf]">Category</div>
        ...
        <div className="border-t border-white/10 px-4 py-3 bg-[#1e1e1e]/50 space-y-2">
            ...
            <div className="flex-1 flex items-center bg-[#161616] border border-white/10 rounded-xl px-3 focus-within:border-[#4edea3] focus-within:ring-1 focus-within:ring-[#4edea3] transition-all">
              ...
              {isSupported && (
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all cursor-pointer ${isListening ? "text-red-400 bg-red-500/10 animate-pulse" : "text-[#bbcabf] hover:text-[#4edea3] hover:bg-[#4edea3]/10"}`}
          ...
          <p className="text-[10px] text-[#bbcabf]/40 text-center font-mono">Try: "I spent $20 on lunch" • Upload a receipt • Ask about your spending</p>
        </div>
      </div>
    </>
  );
