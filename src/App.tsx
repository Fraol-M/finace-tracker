...
import { Settings, Menu, LogOut, User as UserIcon, Sparkles } from "lucide-react";
...
function AppContent() {
  ...
  const getHeaderTitle = () => {
    ...
    switch (activeTab) {
      case "dashboard":
        return "FINPRECISION DASHBOARD";
      case "table":
        return "MONTHLY TRANSACTIONS";
      case "yearly":
        return "YEARLY ANALYTICS";
      case "budget":
        return "SMART BUDGET PLANNER";
      default:
        return "FINPRECISION";
    }
  };
  ...
  return (
    <FinanceProvider userId={currentUser?.id}>
      <div className="min-h-screen bg-[#131313] text-[#e5e2e1] flex relative overflow-x-hidden font-sans select-none antialiased selection:bg-[#4edea3]/20 select-none">
        ...
        <main className="flex-1 flex flex-col md:ml-64 w-full relative min-h-screen pb-12">
          ...
          <header className="w-full sticky top-0 z-30 bg-[#131313]/80 backdrop-blur-xl border-b border-white/5 shadow-sm flex items-center justify-between h-16 px-6 md:px-10">
            ...
            <div className="md:hidden flex items-center gap-3">
              <button onClick={() => setIsMobileSidebarOpen(true)} className="text-[#9ea3a0] hover:text-white transition-colors cursor-pointer">
            ...
            <div className="hidden md:block">
              <h2 className="text-[11px] font-mono tracking-widest text-[#bbcabf] font-semibold uppercase">{getHeaderTitle()}</h2>
            </div>
          ...
          <div className="p-6 md:p-10 max-w-7xl mx-auto w-full flex-1">
            {currentUser.role === "user" && activeTab === "dashboard" && <Dashboard currentUser={currentUser} />}

            {currentUser.role === "user" && activeTab === "table" && <TablePage />}

            {currentUser.role === "user" && activeTab === "yearly" && <YearlyPage />}

            {currentUser.role === "user" && activeTab === "budget" && <BudgetPlanner />}

            {currentUser.role === "admin" && activeTab === "users" && <AdminPage onUpdateUser={handleUpdateUser} />}
          </div>
        </main>
        ...
        {/* AI Chat Panel */}
        {currentUser.role === "user" && <ChatPanel isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} />}
      </div>
    </FinanceProvider>
  );
}